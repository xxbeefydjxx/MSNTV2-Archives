...
function MSNIAIsValidPhonebook(Connector)
{
	...
	for (i = 0; i < Count; i++) {
		...
		if (Entry.NotUserVerified) {
			TVShell.Message("ConnectionManager.js: MSNIAIsValidPhonebook - entry not verified");
...
function SetBYOANBCurrentConnector()
{
	...
	if (!BYOASettings.PrimaryNumber || BYOASettings.PrimaryNumber == "" ||
		...
		TVShell.ConnectionManager.WANAdapter.ErrorCode = ConnectError_DialSettingsInvalid;
		return;
	}
	...
	if (StrippedPhoneNumber.length == 10) {
		// assume they have entered area code, exchange and four digit number but need the 1 prefix
		BYOANBConnector.DialingFlags = BYOANBConnector.DialingFlags |
									DIALFLAG_LONGDISTANCE;
	}
	...
	if (!BYOANBPhonebook.Count) {
		...
		if (BYOASettings.SecondaryNumber != "") {
			...
			if (StrippedPhoneNumber.length == 10) {
				BYOANBPhonebookEntry.Flags |= DIALFLAG_LONGDISTANCE;
			} else {
				BYOANBPhonebookEntry.Flags &= ~DIALFLAG_LONGDISTANCE;
			}
		}
...
function VerifyLANProviderSetup()
{
	...
	if (TVShell.ConnectionManager.EthernetAdapter.Settings.AutoIPEnabled) {
...
function HaveYouMovedPopup()
{
	...
	var str = "<P>If your " + ProductShortName + " is still connected to the phone line " + phone +
		...
		"<P>If your " + ProductShortName + " is connected to a phone line other than " + phone + 
...
function MSNIAError(ErrorCode)
{
	switch (ErrorCode) {
		...
		case ConnectError_DialSettingsStale:
			TVShell.URL = "msntv:/Settings/AccessNumbers.html";
			return true;
	}
