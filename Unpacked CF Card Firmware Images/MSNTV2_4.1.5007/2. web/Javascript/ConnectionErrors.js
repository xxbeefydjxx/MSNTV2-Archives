	...
	else
	{
		switch (ErrorCode)
		{
			...
			case ConnectError_RequireWirelessSettings:
				output = "Wireless settings incomplete";
				break;
...
{	
	...
	var SuggestCheckBYOANumberText = "<LI><SPAN class=li>Check to make sure that you entered the correct telephone number \
									for your Internet Service Provider. </SPAN>";
	
	var SuggestUsingAdditionalBYOANumbersText = "<LI><SPAN class=li>Your Internet Service Provider may have provided you \
											with additional numbers for your local calling area.  \
											You may want to enter different telephone numbers for your "+ProductShortName+" to dial.</SPAN>";
		...
		switch (ErrorCode) {
			...
			case ConnectError_NoDialTone:
				output = "<P>Your "+ProductShortName+" could not connect to the "+NetworkFullName+" because it did not detect a dial tone on your phone line.  " +
					...
					"<UL>" +
						SuggestLiveLineTestText + 
						"<LI><SPAN class=li>If you have voice mail service on your phone line, choose <EM>Settings</EM>; on the Settings page, choose <EM>Basic dialing options</EM> and then choose <EM>Don't wait for dial tone</EM>. </SPAN>\
						</UL>";
				break;
			case ConnectError_Busy:
	...
	else
	{
		switch (ErrorCode)
		{
			...
			case ConnectError_WepKeyIncorrect:
				output = "<P>Your "+ProductShortName+" could not connect to \
						the "+NetworkFullName+" because the wireless privacy key (WEP) \
						was not accepted.  Please verify your wireless settings.";
				break;
			default:
				//if (!fromRegistration)
				//	output = "<P>" + ShortErrorMessage() + "</P>";
...
function ShortErrorMessage()
{
	...
	switch (ErrorCode)
	{
		...
		case ConnectError_RequireWirelessSettings:
			...
			break;
		case ConnectError_WepKeyIncorrect:
			errmsg = "The wireless privacy key was not accepted.";
			break;
		case ConnectError_AssociationFailed:
