...
function CallPaneHelp(hmode)
{
	...
	if (hmode == PH_topicTOC){
	 ...
	 PaneHelpURL = readBaseHelpURL() + "?page=" + escape(arguments[1]);
	 ...
	 PaneHelpURL = readBaseHelpURL() + "?page=" + escape(arguments[1]) + "&amp;retURL=" + encodeURIComponent( curURL );
...
function GetCompatPrinterListURL()
{
 ...
 if(TVShell.UserManager.CurrentUser && TVShell.UserManager.CurrentUser.IsAuthorized)
 {
   PaneHelpURL=GetPaneHelpURL(PH_topicTOC,'MSNTV_PROC_SINGLE_Printing_CompatPrinters.htm');
   return PaneHelpURL
 }
 else
