...
function InitializeOfflineServices()
{
	...
	entry = BuiltinServiceList.Add("Photo::grabber");
	entry.SysCharCode = "w".charCodeAt(0);
	
	
}

// Wholesale customers currently don't have access to Photos, Movies, Videos, Chat, mail and Home Networking
function RemoveFeatures()
{
	var BuiltinServiceList = TVShell.BuiltinServiceList;

	BuiltinServiceList.Remove("Settings::HomeNetwork");

	BuiltinServiceList.Remove("Photo::Home");
	
	BuiltinServiceList.Remove("Photo::Albums");
	
	BuiltinServiceList.Remove("Photo::OnlineStorage");
		
	if (TVShell.Property("MusicFeature")) {
		BuiltinServiceList.Remove("Music::Home");
	}
	
	if (TVShell.Property("VideoFeature")) {
		BuiltinServiceList.Remove("Video::Home");

		// for backward compat w/ video home
		BuiltinServiceList.Remove("Video");
		
		BuiltinServiceList.Remove("Video::HomeNetworking");
	}

	BuiltinServiceList.Remove("Chat");

	BuiltinServiceList.Remove("mail::listmail");
	
	BuiltinServiceList.Remove("media::default");

	BuiltinServiceList.Remove("favorite::shortcut1");

	BuiltinServiceList.Remove("favorite::shortcut2"); 

	BuiltinServiceList.Remove("favorite::shortcut3");

	BuiltinServiceList.Remove("offline::tempMail");
}
