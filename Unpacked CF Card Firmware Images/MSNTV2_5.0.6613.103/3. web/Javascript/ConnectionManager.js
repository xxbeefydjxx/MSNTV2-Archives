...
function ConnectionAttempt(type, date, errorCode, txRate, rxRate, tcpPercentErrors, successfulSignon, phoneNumber) {
	this.type = type;
	this.date = date;
...
function AppendConnectionAttempt(attempt)
{
	...
	var CurrentAttempt = attempt.type + ElementSeparator + attempt.date + ElementSeparator +
		attempt.errorCode + ElementSeparator + attempt.txRate + ElementSeparator + 
		attempt.rxRate + ElementSeparator + attempt.tcpPercentErrors + ElementSeparator + attempt.successfulSignon;
...
{
	var ConnectArray = TVShell.ConnectionManager.ConnectionAttempts.split("!");
	...
	var Attempt = new ConnectionAttempt(AE[0], AE[1], AE[2], AE[3], AE[4], AE[5], AE[6], AE[7], AE[8]);
...
function GetWanTxRate()
{
	var rates;

	if (CurrentWANConnectionType() == ConnectionType_Narrowband) {
		rates = TVShell.ConnectionManager.WANAdapter.Settings.TX_RXRates;
	} else {
		rates = "";
	}
...
function ClearWanTxRxRates()
{
	if (CurrentWANConnectionType() == ConnectionType_Narrowband) {
		TVShell.ConnectionManager.WANAdapter.Settings.TX_RXRates = "";
	}
}
