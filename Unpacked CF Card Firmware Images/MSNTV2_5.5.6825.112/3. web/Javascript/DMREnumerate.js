var audioTypes				= "audio/wav,audio/x-wav,audio/aiff,audio/x-aiff,audio/basic,audio/x-ms-wma,audio/mpeg,audio/mp3";
...
function isUSB( sd )
{
	...
	if ( sd.Provider.toLowerCase() == "local" && !sd.IsNetwork && sd.Mounted && sd.VolumeName.indexOf(":") == 1 && sd.VolumeName.indexOf(userDataPath) != 0)
		bRet = true;
...
function convertBlankSpaces( str )
{
	var result= str;
	if ( str && str != "" ){
		...
		if ( fName != "" )
		{
			...
			if ( topLevelFolder == "" )
			{
				if(parameters["AlbumName"])
				  topLevelViewBy.innerHTML = "You are in <em>" + convertBlankSpaces(parameters["AlbumName"]) + "</em>";
				else
				  topLevelViewBy.innerHTML = "You are in <em>" + convertBlankSpaces(fName) + "</em>";
			}
			else
			{
				
				topLevelViewBy.innerHTML = "View " + homeMediaType + " by " + topLevelFolder + ": <em>" + convertBlankSpaces(fName) + "</em>";
			}
		}
		else
		{
...
function stringToDate(dateStr)
{
	...
	if(isNaN(d))
	{
		var re = /([0-9]{4})-([0-9]{2})-([0-9]{2})T([0-9:]+).*/;
		d = new Date(dateStr.replace(re, "$1/$2/$3 $4"));
		...
		{
			// is dateStr in GMT time?
			if ( dateStr.indexOf("Z") > 0 ){
				var time = d.getTime();
				time -= d.getTimezoneOffset() * 60 * 1000;
				d = new Date( time );
			}
		}
	}	
...
{
	if ( DMRAllowReading == false || ( path == "" && StorageDevice && StorageDevice.Provider.toLowerCase() == "onlinestorage" ))
	{
		if ( myData.userCallback )
			myData.userCallback( myData.userData );
		return;
	...
	if (nodes && nodes.length > 0) {
		...
		for (i = 0; i < length; i++) {
			...
			var timeV =stringToDate(date);
			var timeMs;
			if(!isNaN(timeV))
				timeMs = timeV.getTime();
			var uidNode = node.selectSingleNode(".//a:uid");
			var uid = "";
			if(uidNode) uid = uidNode.text;
			...
			if (href) {
				...
				{
					PlaylistArray.push(new DMRItem(name, href, contentType, myData.checked, date, timeMs, uid ));
				}
				else {
					...
					if ( protectNode && protectNode.text == "Microsoft WM DRM" )
					{
						if ( href.indexOf("?") != -1 )
							href += "&WMDRM=1";
						else
							href += "?WMDRM=1";
						TVShell.Message("Cardea modified href="+href);
					}
					var dmrItem = new DMRItem(name, href, contentType, myData.checked, date, timeMs, uid );
...
function EnumProcessItem()
{
	...
	{
		EnumCurrRequest = req;
		var str = "maxdepth=" + req.maxDepth;
		TVShell.Message("enumerate:" + req.folderPath );
		mystart = new Date().getTime();
		str += ",maxresultcount=" + DMRMaxResultCount;
		str += ",maxsearchcount=" + DMRMaxSearchCount;
		str += ",async=1";
		// store ID for this enumeration
		req.result = req.storageDevice.EnumerateItemsXML( req.folderPath,req.mimeTypes, str );
	}
}

function Enum_OnEnumComplete( storageDevice, result )
{
	var et = new Date().getTime();
	TVShell.Message("!!!!enum took:" + ( et - mystart ) );
	var vn = storageDevice.VolumeName.toLowerCase();
	var req = EnumCurrRequest;
	if ( req && vn == req.volumeName && ( req.result.Id == result.ID ) )
	{
		EnumCurrRequest = null;
		req.func( req.userData, req.storageDevice, req.folderPath, req.mimeTypes, result.XMLDoc , result.Error );
	}
