...
var ConnectError_ConnectedTimeOutWholesale = 44;
var ConnectError_DialNextNumber = 45;
var ConnectError_SlowConnectionRedial = 46;

// from RASError.h
var ERROR_AUTHENTICATION_FAILURE = 691;

// Connection Manager adapter types
var AdapterType_NONE = 0;
var AdapterType_Modem = 1;
var AdapterType_Ethernet = 2;
var AdapterType_Wireless = 3;
...
// WAN Connection Type
var ConnectionType_Narrowband = "narrowband";
var ConnectionType_Broadband = "broadband";
...
var NARROWBAND_BAUDRATE_THRESHOLD_LOW = 4800;
var NARROWBAND_BAUDRATE_THRESHOLD_MEDIUM = 9600;
var NARROWBAND_BAUDRATE_THRESHOLD_HIGH = 19200;

var SLOW_NARROWBAND_DATE_PROPERTY = "SlowNarrowbandTipsDisplayedDate";
...
// FormatDialedNumberWithPrefixes() is for formatting numbers as dialed
function FormatDialedNumberWithPrefixes(phone)
{
	if ( phone.length == 0 )
		return "";
	...
	Splits[Splits.length - 1] = FormatDialedNumber(phone);
	phone = Splits.join(",");

	return phone;
}

// FormatDialedNumber() is for formatting numbers without the prefixes
function FormatDialedNumber(phone)
{
	...
	if ("the " + ServiceShortName + " toll-free number" == phone) {
		return "Toll Free";
	}
...
function RemoveLeadingSpaces(stringToken)
{
	...
	if (stringToken.length) {
		while (" " == stringToken.charAt(0)) {
			stringToken = stringToken.substring(1, stringToken.length);
		}
	}
...
function GetTXRate()
{
	...
	return RemoveLeadingSpaces(splitRates[0]);
}
...
function GetRXRate()
{
	...
	return RemoveLeadingSpaces(splitRates[splitRates.length - 1]);
}
...
function SetSpookyTollFreeNumber(SignUpConnector)
{
	if (!SignUpConnector) {
		return;
	}
	...
	SignUpConnector.DialingFlags = DIALFLAG_TENDIGITDIALING | DIALFLAG_1800SIGNUP;
	SignUpConnector.PhoneNumber = ModemSettings.TollFreeNumber;
	...
	PhonebookEntry.Flags = SignUpConnector.DialingFlags;
	PhonebookEntry.Location = SignUpConnector.Name;
	PhonebookEntry.Number = ModemSettings.TollFreeNumber;

	// Caller must do the ConnectionManager.Save();
}
...
function SetMSNIANBCurrentConnector()
{
	...
	//
	var SpookyTollFree = false;

	if (TVShell.SystemInfo.Flavor != "release" && TVShell.SystemInfo.Flavor != "ppe" && ModemSettings && ModemSettings.TollFreeNumber != "") {
		SpookyTollFree = true;
	}
	...
	for (i = 0; i < Connectors.Count; i++)
		if (Connectors.Item(i).Name  == SignUpConnectorName) {
			if (Connectors.Item(i).CredsPresent) {
				...
				if (SpookyTollFree) {
					SetSpookyTollFreeNumber(Connectors.Item(i));
				}
	...
	//
	var SignUpConnector = SetMSNIANBSignUpConnector(MSNTVServiceList[0]);

	if (SpookyTollFree) {
		SetSpookyTollFreeNumber(SignUpConnector);
		TVShell.ConnectionManager.Save();
	}
...
function LoginToService(serviceEntryName)
{
	...
	// flag this WANConnect as the first in a sequence of WANConnects
	if (TVShell.ConnectionManager.WANProvider == MSNIAModemProviderName && 
		("connection::login" == serviceEntryName || "connection::reconnect" == serviceEntryName ||
		"" == serviceEntryName)) {
		TVShell.Property.Add("RemainingMSNIADialAttempts", "FirstMSNIA_WANConnect");
	} else {
		TVShell.Property.Remove("RemainingMSNIADialAttempts");
	}

	TVShell.ConnectionManager.WANConnect(serviceEntryName);
}

function FirstMSNIA_WANConnect()
{
	...
	if (TVShell.ConnectionManager.WANProvider && TVShell.ConnectionManager.WANProvider != "" &&
		TVShell.ConnectionManager.WANProvider == MSNIAModemProviderName && 
		(RemainingDialAttempts = TVShell.Property.Item("RemainingMSNIADialAttempts")) && 
		RemainingDialAttempts == "FirstMSNIA_WANConnect") {
		TVShell.Property.Remove("RemainingMSNIADialAttempts");
		ReturnValue = true;
	}
...
function InitializeRemainingDialAttempts(ConnectCause)
{
	if (TVShell.ConnectionManager.WANProvider == MSNIAModemProviderName && 
		("connection::login" == ConnectCause || "connection::reconnect" == ConnectCause ||
		"" == ConnectCause)) {
		var PhoneBook;
		var MaxAttempts = 0;

		if (TVShell.ConnectionManager.MSNIAManager.CurrentConnector &&
			TVShell.ConnectionManager.MSNIAManager.CurrentConnector.Phonebook) {
			// if these items don't exist the box can't dial so a handled error will occur and the user will have to retry
			MaxAttempts = MaxDialAttempts(TVShell.ConnectionManager.MSNIAManager.CurrentConnector.Phonebook);
		}

		TVShell.Property.Add("RemainingMSNIADialAttempts", MaxAttempts - 1);
	} else {
		TVShell.Property.Remove("RemainingMSNIADialAttempts");
	}
}

function DecrementRemainingDialAttempts()
{
	var RemainingDialAttempts;
...
function MaxDialAttempts(Phonebook)
{
	var selectedNumbers = 0;
...
function DialedPhoneNumberIsTollFree()
{
	if (!TVShell.ConnectionManager.MSNIAManager.CurrentConnector)
		return false;
...
// This function determines if the box will navigate to the NewPhonebook.html
// page upon disconnecting from the service.
function GoingToNewPhoneBook()
{
	var connector;
	...
	if (connector &&
		connector.Name == LocalConnectorName &&
		connector.CredsPresent &&
		TVShell.ConnectionManager.MSNIASettings.PhonebookID &&
		TVShell.ConnectionManager.MSNIASettings.PhonebookID != "" &&
		TVShell.ConnectionManager.MSNIASettings.PhonebookID != 
		connector.Phonebook.LocationID) {
		return true;
	}
...
// Globals for the connection attempt history
var ElementSeparator = "_";
var AttemptSeparator = "!";
var MaxAttemptsSaved = 8;

// <Narrowband/Broadband>_<Date>_<ErrorCode>_<TX_Rate>_<RX_Rate>_<TCP_Packet_Errors>_<Number>
function ConnectionAttempt(type, date, errorCode, errorDetail, txRate, rxRate, tcpPercentErrors, successfulSignon, duration, phoneNumber) {
	this.type = type;
	this.date = date;
	...
	this.txRate = txRate;
	this.rxRate = rxRate;
	this.tcpPercentErrors = tcpPercentErrors;
	this.successfulSignon = successfulSignon;
	this.duration = duration;

	if ("narrowband" == this.type.toLowerCase() && "undefined" != typeof phoneNumber) {
		this.phoneNumber = phoneNumber;
	}
}

function AppendConnectionAttempt(attempt)
{
	var ConnectString = TVShell.ConnectionManager.ConnectionAttempts;

	var ConnectArray = ConnectString.split(AttemptSeparator);

	switch(attempt.errorCode) {
		case ConnectError_ConnectedTimeOutWholesale:  // Inactivity timeout
		case ConnectError_ConnectedTimeOut:  // Inactivity timeout
		case ConnectError_Reconnect:  // Done repoptimizing
		case ConnectError_BadHomeNumber:  // User error, MSNIA returned no LocalPop ISPs
		case ConnectError_NoHomeNumber:  // User error, either the area code or exchange properties are empty
			attempt.errorCode = ConnectError_NoError;
			break;
		...
		case ConnectError_PowerCycled:
		case ConnectError_BadHomeNumber:
		case ConnectError_BYOA_10_Digit_Upgrade:
		...
		case ConnectError_PhonebookEmpty:
			// Don't log these cases
			return;
			break;
	}

	// If the user is an MSNIA user change ConnectError_AuthFailure to ConnectError_RASError
	// with subcode == ERROR_AUTHENTICATION_FAILURE (691).  This way the MSNIA user
	// won't see the "" error description which may confuse them into thinking that their 
	// Passport username and password are wrong.
	var ErrorCode = attempt.errorCode;
	var ErrorDetail = parseInt(attempt.errorCode) == ConnectError_RASError?attempt.errorDetail:"0";

	if (TVShell.ConnectionManager.WANProvider == MSNIAModemProviderName &&
		ErrorCode == ConnectError_AuthFailure) {
		ErrorCode = ConnectError_RASError;
		ErrorDetail = ERROR_AUTHENTICATION_FAILURE;
	}

	var CurrentAttempt = attempt.type + ElementSeparator + attempt.date + ElementSeparator +
		ErrorCode + ElementSeparator + ErrorDetail + ElementSeparator + 
		attempt.txRate + ElementSeparator + attempt.rxRate + ElementSeparator + 
		attempt.tcpPercentErrors + ElementSeparator + attempt.successfulSignon + ElementSeparator +
		attempt.duration;

	if ("narrowband" == attempt.type.toLowerCase()) {
		CurrentAttempt += ElementSeparator + attempt.phoneNumber;
	} else {
		CurrentAttempt += ElementSeparator + " ";
	}

	if (!ConnectArray || 0 == ConnectArray.length) {
		// make the new entry the only entry in the array
		ConnectArray = new Array();
		ConnectArray[0] = CurrentAttempt;
	} else if (ConnectArray.length >= 1 && ConnectArray.length < MaxAttemptsSaved) {
		// append the new entry
		ConnectArray[ConnectArray.length] = CurrentAttempt;
	} else {
		// delete the first entry and append the new entry
		for (var i = 0; i < ConnectArray.length-1; i++) {
			ConnectArray[i] = ConnectArray[i+1];
		}

		ConnectArray[ConnectArray.length-1] = CurrentAttempt;
	}

	TVShell.ConnectionManager.ConnectionAttempts = ConnectArray.join(AttemptSeparator);
	TVShell.ConnectionManager.Save();
}

function GetConnectionAttempt(index, connectionType)
{
	var ConnectArray = TVShell.ConnectionManager.ConnectionAttempts.split(AttemptSeparator);

	if (!ConnectArray || index > ConnectArray.length - 1) {
		return null;
	}
	...
	for (i=0; i<ConnectArray.length; i++) {
		AttemptEntry = ConnectArray[i];
		AE = AttemptEntry.split(ElementSeparator);

		if (AE[0] == connectionType && AE.length == 10) {
			indexOfType++;
	...
	var Attempt = new ConnectionAttempt(AE[0], AE[1], AE[2], AE[3], AE[4], AE[5], AE[6], AE[7], AE[8], AE[9]);

	return Attempt;
}

function GetConnectionAttemptString(index, connectionType)
{
	var ConnectArray = TVShell.ConnectionManager.ConnectionAttempts.split(AttemptSeparator);
...
function CurrentWANConnectionType()
{
	if (TVShell.ConnectionManager.WANProvider == BYOAEthernetProviderName) {
		return ConnectionType_Broadband;
	} else {
		return ConnectionType_Narrowband;
	}
}

function NumberOfConnectionAttempts(connectionType)
{
	var ConnectString = TVShell.ConnectionManager.ConnectionAttempts;
	var ConnectArray = ConnectString.split(AttemptSeparator);
	var i;
	...
	for(i=0; i<ConnectArray.length; i++) {
		var AE = ConnectArray[i].split(ElementSeparator);
...
function GreatestConnectedBaudRate(connectionType)
{
	var ConnectString = TVShell.ConnectionManager.ConnectionAttempts;
	var ConnectArray = ConnectString.split(AttemptSeparator);
	var i;
	...
	for(i=0; i<ConnectArray.length; i++) {
		var AE = ConnectArray[i].split(ElementSeparator);
		var BaudRate = parseInt(AE[5]);

		TVShell.Message("GreatestConnectedBaudRate() BaudRate = " + BaudRate);

		if (AE[0] == connectionType && BaudRate > result) {
			result = BaudRate;
		}
	}

	TVShell.Message("GreatestConnectedBaudRate() Returning = " + result);
	return result;
}

function FailedToSignOn(AE)
{
	return AE.successfulSignon != "true" && 
		(AE.errorCode != ConnectError_NoError && AE.errorCode != ConnectError_SlowConnectionRedial);
}

function LogConnectionHistoryRecords(attempts)
{
	for (var i=0; i<attempts; i++) {
		var AEString = GetConnectionAttemptString(i, CurrentWANConnectionType());

		if (!AEString) {
			break;
		}

		TVShell.EventLog.Usage("ConnectionHistory","history" + (i+1) + "of" + attempts + "=" + AEString);
		TVShell.Message("LogConnectionHistoryRecords(): Logging Connection Attempts: history" + (i+1) + "of" + attempts + "=" + AEString);
	}
}

function LogPoorConnectionHistory()
{
	var attempts = NumberOfConnectionAttempts(CurrentWANConnectionType());
	var failedAttempts = 0;
	var newestDateString;
	var lastLoggedDateString = TVShell.ConnectionManager.LastLoggedAttemptDate;
	var lastLoggedEntryPresent = false;
	var i;

	if (attempts < MaxAttemptsSaved/2) {
		return;
	}

	for (i=0; i<attempts; i++) {
		var AE = GetConnectionAttempt(i, CurrentWANConnectionType());

		if (!AE) {
			break;
		} else if (FailedToSignOn(AE)) {
			failedAttempts++;
		}

		if (lastLoggedDateString && lastLoggedDateString != "" && lastLoggedDateString == AE.date) {
			lastLoggedEntryPresent = true;
		}

		if (i == attempts-1) {
			newestDateString = AE.date;
		}
	}

	if (failedAttempts >= attempts/2) {
		if (lastLoggedEntryPresent != true) {
			LogConnectionHistoryRecords(attempts);
			TVShell.ConnectionManager.LastLoggedAttemptDate = newestDateString;
			TVShell.ConnectionManager.Save();
		}
	}
}

function GetWanTxRate(ratesString)
{
	var rates;
...
function GetWanRxRate(ratesString)
{
	var rates;
...
function GetSlowNarrowbandTipsDisplayedDate()
{
	var PersistentProperties = TVShell.Property("Persistent/");
	var returnValue;

	if (PersistentProperties && PersistentProperties(SLOW_NARROWBAND_DATE_PROPERTY)) {
		returnValue = PersistentProperties(SLOW_NARROWBAND_DATE_PROPERTY);
	} else {
		returnValue = "";
	}
...
function SetSlowNarrowbandTipsDisplayedDate()
{
	var PersistentProperties = TVShell.Property("Persistent/");
	var CurrentDate = new Date();

	PersistentProperties.Add(SLOW_NARROWBAND_DATE_PROPERTY, CurrentDate.toString());
	PersistentProperties.Save();
}

function TimeToDisplayNarrowbandTips()
{
	var LastDateDisplayedString = GetSlowNarrowbandTipsDisplayedDate();
	var ReturnValue = false;

	if (!TVShell.DeviceControl.ClockSet) {
		TVShell.Message("TimeToDisplayNarrowbandTips(): Clock not set so returning");
		return false;
	}

	if (!LastDateDisplayedString || "" == LastDateDisplayedString) {
		TVShell.Message("TimeToDisplayNarrowbandTips(): LastDateDisplayedString is empty, displaying tips and setting date");
		return true;
	}

	var LastDateDisplayed = new Date(LastDateDisplayedString);

	if (LastDateDisplayed.getFullYear() == 2003) {
		// This should not happen
		SetSlowNarrowbandTipsDisplayedDate();
		TVShell.Message("TimeToDisplayNarrowbandTips(): LastDateDisplayed.getFullYear() = 2003, setting date but not displaying tips");
		return false;
	}

	var CurrentDate = new Date();

	TVShell.Message("TimeToDisplayNarrowbandTips(): CurrentDate.toLocaleString() = " + CurrentDate.toLocaleString());
	TVShell.Message("TimeToDisplayNarrowbandTips(): LastDateDisplayed.toLocaleString() = " + LastDateDisplayed.toLocaleString());

	var DurationInDays;

	if (TVShell.SystemInfo.Flavor != "release" && TVShell.SystemInfo.Flavor != "ppe") {
		// for testing it is actually in 10 second intervals so 30 == 5 minutes
		DurationInDays = Math.floor((CurrentDate.getTime() - LastDateDisplayed.getTime())/(10*1000));
	} else {
		DurationInDays = Math.floor((CurrentDate.getTime() - LastDateDisplayed.getTime())/(1000*60*60*24));  // days
	}

	if (DurationInDays >= 30) {  // 30 days
		ReturnValue = true;
	}

	TVShell.Message("TimeToDisplayNarrowbandTips(): ReturnValue = " + ReturnValue);
	return ReturnValue;
}

function GetNarrowbandBaudrateThresholdLow()
{
	var PersistentProperties = TVShell.Property("Persistent/");
	var BaudRateThresholdLow = PersistentProperties("BaudRateThresholdLow");

	if (!BaudRateThresholdLow || BaudRateThresholdLow <= 0) {
		BaudRateThresholdLow = NARROWBAND_BAUDRATE_THRESHOLD_LOW;
	}

	return parseInt(BaudRateThresholdLow);
}

function GetNarrowbandBaudrateThresholdMedium()
{
	var PersistentProperties = TVShell.Property("Persistent/");
	var BaudRateThresholdMedium = PersistentProperties("BaudRateThresholdMedium");

	if (!BaudRateThresholdMedium || BaudRateThresholdMedium <= 0) {
		BaudRateThresholdMedium = NARROWBAND_BAUDRATE_THRESHOLD_MEDIUM;
	}

	return parseInt(BaudRateThresholdMedium);
}

function GetNarrowbandBaudrateThresholdHigh()
{
	var PersistentProperties = TVShell.Property("Persistent/");
	var BaudRateThresholdHigh = PersistentProperties("BaudRateThresholdHigh");

	if (!BaudRateThresholdHigh || BaudRateThresholdHigh <= 0) {
		BaudRateThresholdHigh = NARROWBAND_BAUDRATE_THRESHOLD_HIGH;
	}

	return parseInt(BaudRateThresholdHigh);
}
