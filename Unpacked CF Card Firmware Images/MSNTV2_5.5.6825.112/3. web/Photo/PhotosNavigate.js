...
var ATTACHMENT_LIST_FILENAME = tempDir + "\\photo\\TempSelectedPhotoList.txt";
var IDOK=1;
var IDCANCEL=2;
...
if (fromMail)
{
	...
	if(AttachmentFileList)
	{
		...
		var fileName;
		var dotPos;
		var reWebready = /^[.*]\\photo\\resizedcache/g; 
		var reRotated = /^[.*]\\photo\\rotated/g; 
...
{
	if (indexMap==null && DMRItemArray != null  && nPhotos>0 )
	{
		indexMap = new Object();
...
function SelectedOnlinePrintList( list )
{
	var fileList = new Array();
	var j=0;
	var theURL;
	...
	if(RootNode)
	{
		...
		if (nodes && nodes.length > 0) 
		{
			...
			for (i = 0; i < length; i++) 
			{
				...
				var name = webReadyURL ? decodeURIComponent(ExtractFileName(webReadyURL)) : null;
				
				DMRItemArray[i] = new DMRItem(name, webReadyURL, "image/jpeg", "", date, timeMs,  webReadyURL, thumbnailURL, printReadyURL, from, mailURL, subject);
			}
...
function BuildArraysFromAllImageListFile()
{
	forceSelectable = true;
	var st = new Date().getTime();
    g_fTruncated = false;
	var ImageListFilePath = userDataVolumeName+"\\"+PhotoSettingsPath+"\\PageImageList.txt";
	var data=TVShell.Utilities.ReadTextFile(ImageListFilePath);
	...
	for ( var i = 0; i < urls.length; i++)
	{
		...
		var contentType = SupportedPhotoMIMETypes;
		var timeV = new Date();
		var date = "" + timeV;
		var timeMs = timeV.getTime();
		...
		if (href) 
		{
			var dmrItem = new DMRItem(name, href, contentType,false, date , timeV.getTime() );
			DMRItemArray.push( dmrItem );
			imgCount++;
		}
	}
	
	TVShell.Message("Found "+imgCount+" image(s)");
	CheckedItemCount = 0;
	nPhotos = DMRItemArray.length;

	var et = new Date().getTime();

	TVShell.Message( "BuildArraysFromAllImageListFile. time=" + (et-st) );
	if(DMRItemArray && DMRItemArray.length>0)
	{
		DMRItemArray.sort(SortItemsByTime);
		GenerateGroups();
...
{
	//TVShell.Message("unformatted date = " + dateStr);
	var result = UndatedString;			
	var d=stringToDate(dateStr);
	...
	{	
		if(currentGranularity=="year")
			...
			result = monthList[d.getMonth()] + ", " + d.getYear();
		else
			result = d.getDate() + " " + monthList[d.getMonth()] + ", " + d.getYear();
	}

	//TVShell.Message("formatted date = " + result);
	return result;
}
...
function IsOnlineStorageDevice()
{	
...
var isOnlineDevice = null;
function IsOnlineDevice()
{
	if ( isOnlineDevice == null )
	{
		isOnlineDevice = false;
		if( IsOnlineStorageDevice() || parameters.AttachmentViewer || parameters.XMLFileURL)
			isOnlineDevice = true;
	}
...
function AskQuality()
{
	...
	var retVal = (isLocal || IsOnlineStorageDevice()) ? 1 : PanelManager.CustomMessageBox(
...
function SendPhotos(quality, action)
{
	...
	SetProgressText(L_PROGRESS_PLEASE_WAIT + progressMsg );
...
function SendPhotos2(srcURL)
{
	...
	try
	{
		...
		else if(location==2 && (quality == RES_FULL))
		{
		    // unlike other online photo sources, client has to upload photos on WMC itself
		    // However, PicturePusher works on local files only and not with network files.
		    // The friendly resized file is already a local one. For "Full" photo, we first 
		    // make a local copy of the photo and then upload this one.
		    
		    var dstURL = "\\photo" + PhotoManager.GetFileName(srcURL);
    		if(ThumbnailManager.CopyFileToURL(srcURL, dstURL))
    		    srcURL = dstURL;
    		else
    		    TVShell.Message( "Unable to copy file from WMC " + orig );  
...
function OnDropDownSelect()
{
	var subFrame = window.frames['scrollListIFrame'];
	var subDoc = subFrame.document;
    subDoc.body.scrollTop = subDoc.all["group" + dropDown.selectedIndex].offsetTop;
}
